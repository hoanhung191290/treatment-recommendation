...
indFeatures = list(drugInfo.columns[3:])


infoDf = pd.DataFrame(columns=['subject_id', 'score', 'proportionMainDrug', 'proportionSymptomDrug', 'proportionRiskDrug', 'prescribedTimes', 'mileStone1', 'mileStone2', 'trainIdx', 'testIdx', 'clusterIdx','neighbors','clusterIdxCombine','clusterIdx2']+indFeatures)
print('done')
patientIdxes = []
...
data_symp = genfromtxt('data/derived/mixed.csv')
prescDf = pd.read_csv('data/preprocessed/new_prescription.csv')
periodEncodeDf = scoringWithIndication.deriveScore(infoDf=infoDf, prescDf=prescDf)
...
for testSize in testSizes:
    ...
    precisions = [[[] for x in range(h)] for y in range(w)]
    recalls = [[[] for x in range(h)] for y in range(w)]
    ...
    for i in range(nTests):
        ...
        if(view == 'symptom'):
            ...
            tFeatures = infoDf.columns[14:]
            for t in tFeatures:
                infoDf[t] = infoDf[t].astype(float)

            transferedData = infoDf.as_matrix(tFeatures)
        ...
        infoDf.to_csv('data/derived/infoDf/'+str(testSize)+'_'+str(i)+'.csv', header=True, index = False)
        ...
        for idxDep in range(len(depths)):
            precisions[idxNei][idxDep] = []
            recalls[idxNei][idxDep] = []
            ...
            for idxNei in range(len(nNeighbors)):
                ...
                for t in testIdxes:
                    ...
                    for j in range(3):
                        ...
                        print('neighbor drugs....'+str(neighborDrugList))
                        ...
                        if (len(neighborDrugs) != 0):
                            precision += 1.0 * len(nCorrected) / depths[idxDep]
                            recall += 1.0 * len(nCorrected) / len(set(testPatientDrugs))
                ...
                precision = precision / (1.0 * nPeriods * len(testIdxes))
                recall = recall / (1.0 * nPeriods * len(testIdxes))
                correct = np.sum(np.multiply(scores, range(4)))
                precisions[idxNei][idxDep].append(precision)
                recalls[idxNei][idxDep].append(recall)
                corrects[idxNei][idxDep].append(correct/(3 * len(testIdxes)))
                wFiles[idxNei][idxDep].write('\tno correct paths: {}, precision: {}, recall: {} \n'.format(correct, precision, recall))
    ...
    for idxNei in range(len(nNeighbors)):
        for idxDep in range(len(depths)):
            ...
            cor = np.mean(corrects[idxNei][idxDep])
            corVar = np.var(corrects[idxNei][idxDep])
            pre = np.mean(precisions[idxNei][idxDep])
            preVar = np.var(precisions[idxNei][idxDep])
            rec = np.mean(recalls[idxNei][idxDep])
            recVar = np.var(recalls[idxNei][idxDep])
            f1_s = [2.0 * precisions[idxNei][idxDep][i] * recalls[idxNei][idxDep][i] / (precisions[idxNei][idxDep][i] +recalls[idxNei][idxDep][i]) for i in range(len(precisions[idxNei][idxDep]))]
            f2_s = [2.0 * precisions[idxNei][idxDep][i] * corrects[idxNei][idxDep][i] / (precisions[idxNei][idxDep][i] +corrects[idxNei][idxDep][i]) for i in range(len(precisions[idxNei][idxDep]))]
            ...
            #f3 = (3.0 * pre * rec * cor) / (pre + rec + cor)
            wFiles[idxNei][idxDep].write('\n\tcorrect_bar: {} var: {} original: {} \n'.format(cor, corVar, corrects[idxNei][idxDep]))
            wFiles[idxNei][idxDep].write('\n\tprecision_bar: {} var: {} original: {} \n'.format(pre, preVar, precisions[idxNei][idxDep]))
            wFiles[idxNei][idxDep].write('\n\trecall_bar: {} var: {} original: {} \n'.format(rec, recVar, recalls[idxNei][idxDep]))
            wFiles[idxNei][idxDep].write('\n\tf1 score: {} var: {} original: {} \n'.format(f1, f1Var, f1_s))
            wFiles[idxNei][idxDep].write('\n\tf2 score: {} var: {} original: {} \n'.format(f2, f2Var, f2_s))
            #wFiles[idxNei][idxDep].write('\n\tf3 score: {}\n'.format(f3))
            wFiles[idxNei][idxDep].flush()
for idxNei in range(len(nNeighbors)):
    for idxDep in range(len(depths)):
